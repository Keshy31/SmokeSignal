...
interface DirectFireAlertsListProps {
  onAlertMe: (fireId: string) => void;
}

// Define the filter options
type FilterOption = 'all' | 'nearby' | 'severe' | 'recent' | 'visible' | 'province';
...
const DirectFireAlertsList: React.FC<DirectFireAlertsListProps> = ({ onAlertMe }) => {
  const { map } = useMapContext();
  const { location, error: locationError } = useGeolocation();
  const { t } = useTranslation();

  // Add states for fire data
  ...
  const fetchFires = async () => {
    try {
      ...
      console.log('Fetched fires directly:', data.length);

      // Add distance calculations if user location is available
      const firesWithDistances = data.map((fire: FireLocation) => {
        ...
        if (location && location.latitude && location.longitude) {
          ...
          fireWithDistance.distance = "Unknown";
        }
        return fireWithDistance;
      });

      setFires(firesWithDistances);
      setFilteredFires(firesWithDistances); // Initialize filtered fires too
      setError(null);
    } catch (err) {
  ...
  // Fetch fires on component mount
  useEffect(() => {
    fetchFires();
  }, []);
  ...
  useEffect(() => {
    if (location && fires.length > 0) {
      const updatedFires = fires.map(fire => {
        const distanceKm = calculateDistance(
          location.latitude,
          location.longitude,
          fire.latitude,
        ...
        return { ...fire, distance: distanceKm.toFixed(1) };
      });
      setFires(updatedFires);
      // Also update filtered fires to maintain consistency
      const currentFiltered = applyFiltersAndSort(updatedFires);
      setFilteredFires(currentFiltered);
    }
  }, [location]);
  ...
  const applyFiltersAndSort = (firesToFilter: FireLocation[]): FireLocation[] => {
    let result = [...firesToFilter];
    ...
    return sortFires(result);
  };

  // Apply filtering and sorting whenever dependencies change
  useEffect(() => {
    if (fires.length > 0) {
      console.log('Applying filters to', fires.length, 'fires');
      const filtered = applyFiltersAndSort(fires);
      setFilteredFires(filtered);
    }
  }, [fires, activeFilter, sortBy, maxDistance, selectedProvince, map]);

  // Determine what to render based on loading state and data availability
  if (isLoading) {
    return (
      <div className="bg-app-dark-lighter rounded-xl p-4 shadow-md mb-6 text-center">
        <p className="text-gray-300">{t('fire.loadingFires') || 'Loading fire data...'}</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-app-dark-lighter rounded-xl p-4 shadow-md mb-6 text-center text-red-400">
        <AlertCircle className="mx-auto mb-2" />
        <p>Error loading fires: {error.message}</p>
        <Button 
          variant="outline" 
          size="sm" 
          onClick={fetchFires} 
  ...
  if (fires.length === 0) {
    return (
      <div className="bg-app-dark-lighter rounded-xl p-4 shadow-md mb-6 text-center">
        <p className="text-gray-300">No active fires found.</p>
      </div>
    );
  }

  // Log for debugging
  console.log(`Rendering ${filteredFires.length} filtered fires out of ${fires.length} total fires`);
            ...
            <Tabs defaultValue="filter" className="w-full">
              ...
              <TabsContent value="filter" className="space-y-3">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <Button 
                    variant={activeFilter === 'visible' ? 'default' : 'outline'} 
                    size="sm"
                    onClick={() => setActiveFilter('visible')}
                    className="w-full"
                  >
                    {t('fire.filterOptions.visible')}
                  </Button>
                  <Button 
                    variant={activeFilter === 'province' ? 'default' : 'outline'} 
                    size="sm"
                    onClick={() => setActiveFilter('province')}
                    className="w-full"
                  >
                    {t('fire.filterOptions.province') || 'By Province'}
                  </Button>
                </div>
                ...
                {activeFilter === 'province' && (
                  <div className="pt-2">
                    <div className="text-xs text-gray-400 mb-1">
                      <span>{t('fire.selectProvince')}:</span>
                    </div>
                    <Select
                      value={selectedProvince}
                      onValueChange={(value) => setSelectedProvince(value as ProvinceOption)}
                    >
                      <SelectTrigger className="w-full bg-gray-800 border-gray-700">
                        <SelectValue placeholder={t('fire.allProvinces')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">{t('fire.allProvinces')}</SelectItem>
                        <SelectItem value="western-cape">{t('fire.province.westernCape') || 'Western Cape'}</SelectItem>
                        <SelectItem value="eastern-cape">{t('fire.province.easternCape') || 'Eastern Cape'}</SelectItem>
                        <SelectItem value="northern-cape">{t('fire.province.northernCape') || 'Northern Cape'}</SelectItem>
                        <SelectItem value="kwazulu-natal">{t('fire.province.kwazuluNatal') || 'KwaZulu-Natal'}</SelectItem>
                        <SelectItem value="free-state">{t('fire.province.freeState') || 'Free State'}</SelectItem>
                        <SelectItem value="gauteng">{t('fire.province.gauteng') || 'Gauteng'}</SelectItem>
                        <SelectItem value="north-west">{t('fire.province.northWest') || 'North West'}</SelectItem>
                        <SelectItem value="mpumalanga">{t('fire.province.mpumalanga') || 'Mpumalanga'}</SelectItem>
                        <SelectItem value="limpopo">{t('fire.province.limpopo') || 'Limpopo'}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                )}
        ...
        <div className="space-y-3">
          {filteredFires.map((fire) => {
            ...
            return (
              <div 
                key={fire.id} 
                className="bg-gray-900/70 border border-gray-700/40 rounded-xl p-4 shadow-lg hover:border-gray-600/80 transition-colors backdrop-blur-sm cursor-pointer"
                onClick={() => {
                  console.log('Fire card clicked:', fire.location);
...
export default DirectFireAlertsList;